...
const ScrollBar: React.FC<ScrollBarProps> = (props: ScrollBarProps) => {
	...
	const [state, setState] = useState(initScrollBarState as ScrollBarState);
	useEffect(()=>{
		/**
		 * ... here is for componentDidMount and componentDidUpdate will trigger this
		 * */
		const scrollerHeight: number = wrapper.current && content.current ? content.current?.clientHeight / wrapper.current?.clientHeight : 0;
	...
	return (
		<div className={"ScrollBar"}>
			...
			<div className={"ScrollContainer"}>
				<div className={"Scroller"} onMouseDown={(e) => onMouseDown(e)} onMouseOver={(e)=>onMouseOver(e)}/>
			</div>
		</div>
	);
};
