...
function customFilter(_data) {
	...
	data = data.filter(value => !/{address}/g.test(value));
	data = data.filter(value => !/this.votingMessage\(proposal\)/g.test(value));
	data = data.filter(value => !/this.formatVotes\(proposal.myVotes\)/g.test(value));
	data = data.filter(value => !/{ asset.symbol }/g.test(value));
	data = data.filter(value => !/{ pool.name }/g.test(value));
	data = data.filter(value => !/{ address }/g.test(value));
	data = data.filter(value => !/asset.claimableBalance/g.test(value));
	data = data.filter(value => !/rewardPool.name/g.test(value));
	data = data.filter(value => !/contract.name/g.test(value));
	data = data.filter(value => !/option.symbol/g.test(value));
	data = data.filter(value => !/quoteContract/g.test(value));
	data = data.filter(value => !/quote.amount/g.test(value));
	data = data.filter(value => !/option.symbol/g.test(value));
	data = data.filter(value => !/contract.name/g.test(value));
	data = data.filter(value => !/contract.coverAmount/g.test(value));
...
function f(files) {
	...
	const formatJSON = JSON.stringify(jsons).replace(/{/, "{\n\040\040").replace(/"}/, "\"\n}").replace(/",/g, "\",\n\040\040");
