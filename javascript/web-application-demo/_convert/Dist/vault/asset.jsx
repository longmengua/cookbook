//i18n => {"vault-vault-Text-1":"This project under construction, please do not use it..","vault.vault.Text.2":"Connect your wallet to continue","vault-vault-Text-3":"This project under construction, please do not use it..","vault.vault.Text.4":"{ asset.name }","vault.vault.Text.5":"{ asset.description }","vault.vault.Text.6":" on ","vault.vault.Text.7":"Not Available","vault.vault.Text.8":"{ option.description }"}
...
class Farming extends Component {
	...
	render() {
		...
		if(!account || !account.address) return <div />;
						...
						<div className={ classes.assetSummary }>
							...
							</div>
							{
								...
								<div className={classes.headingEarning}>
									<Typography variant={ 'h5' } className={ classes.grey } id={"vault.vault.Text.14"}>{i18next.t("vault.vault.Text.14")}:</Typography>
									<div className={ classes.flexy }>
										<Typography variant={ 'h3' } noWrap>{ (this._getAPY(asset)/1).toFixed(2) }% </Typography>
										<Typography variant={ 'h5' } className={ classes.on } id="vault.vault.Text.6" >{i18next.t('vault.vault.Text.6')}</Typography>
							...
							}
							{
								(!['LINK'].includes(asset.id) && asset.vaultBalance === 0) &&
								<div className={classes.headingEarning}>
									<Typography variant={ 'h5' } className={ classes.grey }id={"vault.vault.Text.15"} >{i18next.t("vault.vault.Text.15")}:</Typography>
									<div className={ classes.flexy }>
										<Typography variant={ 'h3' } noWrap>{ (this._getAPY(asset)/1).toFixed(2) }% </Typography>
									</div>
								</div>
							}
							{
								...
								<div className={classes.headingEarning}>
									<Typography variant={ 'h5' } className={ classes.grey }>{i18next.t('vault.vault.Text.17')}:</Typography>
									<Typography variant={ 'h3' } noWrap id="vault.vault.Text.7" >{i18next.t('vault.vault.Text.7')}</Typography>
								</div>
							}
							<div className={classes.heading}>
								<Typography variant={ 'h5' } className={ classes.grey }>{i18next.t('vault.vault.Text.16')}:</Typography>
		...
		return (
			<div className={ classes.filters }>
				<FormControlLabel
					...
					label={i18next.t("vault.vault.Text.6")}
				/>
				<div className={ classes.between }>
					<Tooltip title={
						<React.Fragment>
							<Typography variant={'h5'} className={ classes.fees }>{i18next.t("vault.vault.Text.18")}</Typography>
						</React.Fragment>
					} arrow>
						<InfoIcon />
					</Tooltip>
				...
				<TextField
					...
					id={ 'search vault.vault.Text.7' }
					value={ search }
					error={ searchError }
					onChange={ this.onSearchChanged }
					placeholder={i18next.t("vault.vault.Text.7")}
					variant="outlined"
					InputProps={{
	...
	renderBasedOn = () => {
		...
		const options = [
			{
				value: 1,
				description: i18next.t("vault.vault.Text.9")
			},
			{
				value: 2,
				description: i18next.t("vault.vault.Text.10")
			},
			{
				value: 3,
				description: i18next.t("vault.vault.Text.11")
			}
		];
		...
		return (
			<div className={ classes.basedOnContainer }>
				...
				<Typography id={"vault.vault.Text.8"}>{i18next.t("vault.vault.Text.8")}{ basedOn === 4 ? i18next.t("vault.vault.Text.12") : i18next.t("vault.vault.Text.13") }</Typography>
				<TextField
					id={ 'basedOn' }
					name={ 'basedOn' }
					select
